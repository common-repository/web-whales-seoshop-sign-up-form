<?php
	/**
	 * Plugin Name: Lightspeed eCommerce Sign Up Form by Web Whales
	 * Description: This plugin provides an affiliate sign up form for Lightspeed eCommerce Partners. Not yet a Lightspeed eCommerce Partner? Request a partnership for free at <a href="https://resellers.lightspeedhq.com/?utm_source=Web%20Whales&utm_medium=referral&utm_campaign=Web%20Whales%20Lightspeed%20eCommerce%20Sign%20Up%20WordPress%20plugin" target="_blank">Lightspeed</a>.
	 * Version: 1.1
	 * Author: Web Whales
	...
	/**
	 * Class WW_Lightspeed_eCom_Sign_Up
	 */
	final class WW_Lightspeed_eCom_Sign_Up {

		const PLUGIN_PREFIX = 'ww_seoshop_sign_up_', PLUGIN_SLUG = 'ww-seoshop-sign-up', PLUGIN_VERSION = '1.1', TEXT_DOMAIN = 'ww-seoshop-sign-up';
		...
		/**
		 * Format a Lightspeed eCommerce dashboard sign in URL
		 *
		...
		public function page_settings() {
			if ( ! empty( $_POST['action'] ) && $_POST['action'] == 'save_seoshop_settings' && check_admin_referer( 'save-seoshop-settings' ) ) {
				...
				if ( $updated ) {
					self::$notices['update'][] = '<p>' . __( 'Your Lightspeed eCommerce settings have been updated.', self::TEXT_DOMAIN ) . '</p>';
				}
			}

			self::$notices['update'][] = '<p><strong>' . __( 'Important!', self::TEXT_DOMAIN ) . '</strong> ' . sprintf( __( 'You can use this plugin when you are a registered Lightspeed eCommerce Partner only. Not yet a Lightspeed eCommerce Partner? Request a partnership for free at %s.', self::TEXT_DOMAIN ), '<a href="' . __( 'https://resellers.lightspeedhq.com/?utm_source=Web%20Whales&utm_medium=referral&utm_campaign=Web%20Whales%20Lightspeed%20eCommerce%20Sign%20Up%20WordPress%20plugin', self::TEXT_DOMAIN ) . '" target="_blank">Lightspeed</a>' ) . '</p>';
		...
		public function register_shortcodes() {
			add_shortcode( 'seoshop_sign_up_form', array( '\WW_Lightspeed_eCom_Sign_Up', 'do_shortcode_seoshop_sign_up_form' ) );
		}
		...
		public function wp_admin_menu() {
			add_options_page( __( 'Lightspeed Affiliate Settings', self::TEXT_DOMAIN ), __( 'Lightspeed Affiliate Settings', self::TEXT_DOMAIN ), 'manage_options', self::PLUGIN_SLUG, array(
				&$this,
				'page_settings'
			) );
		}
		...
		private function action_process_sign_up_form() {
			...
			if ( $this->check_plugin_settings() ) {
				...
				if ( empty( $result['error_fields'] ) && ! empty( $form_data['form_token'] ) ) {
					...
					if ( ! empty( $sign_up_form['response'] ) && substr( $sign_up_form['response'], 0, 1 ) == '2' && ! empty( $sign_up_result ) ) {
						...
						if ( ! empty( $sign_up_result['success'] ) && ! empty( $sign_up_result['validated'] ) && ! empty( $sign_up_result['data']['user']['id'] ) && ! empty( $sign_up_result['data']['user']['login_hash'] ) ) {
							...
							$result['message'] = '<strong>' . __( 'Congratulations! Your Lightspeed eCommerce store has been created.', self::TEXT_DOMAIN ) . '</strong><br /><br />';

							if ( ! empty( $sign_up_result['form']['fields']['password']['value'] ) ) {
								$result['message'] .= sprintf( __( 'The password for your Lightspeed account is %s.', self::TEXT_DOMAIN ), '<strong>' . $sign_up_result['form']['fields']['password']['value'] . '</strong>' ) . ' ';
							}

							$result['message'] .= __( 'Please use the following link to go to your Lightspeed eCommerce Dashboard.', self::TEXT_DOMAIN );
				...
				$result['message'] = '<strong>' . __( 'Error:', self::TEXT_DOMAIN ) . '</strong> ' . __( 'The Lightspeed eCommerce sign up settings are not set up correctly.', self::TEXT_DOMAIN );
			}
		...
		/**
		 * Get a valid form token from Lightspeed
		 *
		...
		public static function do_shortcode_seoshop_sign_up_form( $args, $content = null ) {
			if ( self::check_plugin_settings() ) {
				...
				return '<em style="background:#EBEBEB;border:1px solid #E0E0E0;border-radius:7px;display:block;margin:20px auto;padding:20px;width:95%;"><strong>' . __( 'Error:', self::TEXT_DOMAIN ) . '</strong> ' . __( 'The Lightspeed eCommerce sign up settings are not set up correctly.', self::TEXT_DOMAIN ) . '</em>';
			}
		}
	 ...
	 */
	add_action( 'plugins_loaded', array( 'WW_Lightspeed_eCom_Sign_Up', 'get_instance' ), 99 );


	/**
	 * Register the activation hook
	 */
	register_activation_hook( __FILE__, array( 'WW_Lightspeed_eCom_Sign_Up', 'plugin_activate' ) );